...
state_vars_dict = defaultdict(list)
...
for ast_dir in tqdm(ast_dirs):
    ...
    for json_file in json_files:
        ...
        for var in state_variables:
            state_vars_dict[(var['name'], str(var['typeName']))].append((ast_dir, json_file, relationship_map))

# 遍历每个状态变量
for state_var, files in state_vars_dict.items():
    # 遍历每个文件
    for ast_dir, json_file, relationship_map in files:
        # 如果"State"文件夹中对应的目录不存在，则创建它
        os.makedirs(os.path.join("State", ast_dir), exist_ok=True)

        # 构造"State"文件夹中CSV文件的路径
        csv_file = os.path.join("State", ast_dir, json_file.replace(".json", ".csv"))
